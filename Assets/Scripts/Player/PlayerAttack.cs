...
public class PlayerAttack : MonoBehaviour
{
    ...
    [SerializeField] PrototypeAttack prototypeAttack;   //Reference to a prototype attack script
    ...
    int numberOfAttacks;                                //The number of attacks the player has
    ...
    void Start()
    {
        if (lightningAttack != null) numberOfAttacks++;
        if (frostAttack != null) numberOfAttacks++;
        if (stinkAttack != null) numberOfAttacks++;
        if (slimeAttack != null) numberOfAttacks++;
        if (prototypeAttack != null && numberOfAttacks == 0)
        {
            numberOfAttacks = 1;
            GameManager.Instance.isIncompleteGame = true;
        }
    }
    ...
    void PrototypeAttack()
    {
        //If there is no lightning attack, leave
        if (prototypeAttack == null)
            return;

        //Fire lightning
        prototypeAttack.Fire();
        //Record the cooldown of the lightning attack
        attackCooldown = prototypeAttack.Cooldown;
