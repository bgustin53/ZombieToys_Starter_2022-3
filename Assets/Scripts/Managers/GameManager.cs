...
public class GameManager : MonoBehaviour
{
    ...
    [SerializeField] TextMeshProUGUI infoText;		//Text UI element to show info to the player (info may be instructions or the player's score)
    [SerializeField] TextMeshProUGUI gameoverText;  //Text UI element informing the player that they have lost
    [SerializeField] GameObject titleCanvas;        //Canvas containing title
    ...
    public bool isIncompleteGame { get; set; }
    ...
    public void StartGame()
    {
        ...
        // If the player selection feature is not yet implemented, it will start the game with the start game button
        if (infoText == null && Player != null)
        {
            PlayerChosen(Player);
        }
        else
        {
            infoText.text = "Select a character";
        }

    }
    //Reference by the Tutorial button on the title screen
    public void DisplayTutorial()
    {
        titleCanvas.transform.GetChild(1).gameObject.SetActive(true); //Tutorial text must be 2nd child of Title Canvas
    }
}
